...
eu_plots_only <- get_param("eu_plots_only", FALSE)

cat("\nEU run mode:\n")
cat("Plots only:", eu_plots_only, "\n")

# ============================================================
# CHECK REQUIRED HELPERS
# ============================================================

required_eu_functions_common <- c(
  "eu_prepare_data",
  "eu_get_targets",
...
required_eu_functions <- if (isTRUE(eu_plots_only)) {
  required_eu_functions_common
} else {
  c(required_eu_functions_common, required_eu_functions_full)
}
...
eu_lag_season <- get_param(
  "eu_lag_season",
  get_param("eu_transform_lag_season", 4L)
)
...
eu_oos_end   <- get_param("eu_oos_end",   as.Date("2025-12-31"))
...
eu_training_obs <- get_param(
  "eu_training_obs",
  get_param("eu_training_quarters", 10L * 4L)
)
...
eu_save_raw_series_plot <- get_param("eu_save_raw_series_plot", TRUE)
eu_save_transformed_series_plot <- get_param("eu_save_transformed_series_plot", TRUE)
eu_save_individual_target_plots <- get_param("eu_save_individual_target_plots", TRUE)

eu_plot_series_scope <- get_param("eu_plot_series_scope", "all")
...
eu_dir <- get_param("eu_dir", here::here("outputs", "eu"))
...
all_transformed_series <- setdiff(names(panel_eu), "Date")

missing_labels <- setdiff(all_transformed_series, names(eu_raw_plot_labels))

if (length(missing_labels) > 0) {
  eu_raw_plot_labels <- c(
    eu_raw_plot_labels,
    stats::setNames(missing_labels, missing_labels)
  )
}

if (isTRUE(eu_save_individual_target_plots)) {
  eu_plot_target_series(
    panel_data = panel_eu,
  ...
  cat("\nSaved EU individual target plots to:\n")
  cat(eu_target_plot_dir, "\n")
}

if (identical(eu_plot_series_scope, "all")) {
  eu_plot_targets_use <- all_transformed_series
} else {
  eu_plot_targets_use <- intersect(eu_target_order, names(panel_eu))
}

eu_raw_plot_file <- file.path(
  eu_plots_dir,
  paste0("all_raw_series_", eu_dataset_label, ".pdf")
)

eu_transformed_plot_file <- file.path(
  eu_plots_dir,
  paste0("all_transformed_series_", eu_dataset_label, ".pdf")
)

if (isTRUE(eu_save_raw_series_plot)) {
  eu_plot_series_combined(
    panel_data = eu_raw_panel,
    targets = intersect(eu_plot_targets_use, names(eu_raw_panel)),
  ...
  cat("\nSaved EU raw series plot to:\n")
  cat(eu_raw_plot_file, "\n")
}

if (isTRUE(eu_save_transformed_series_plot)) {
  eu_plot_series_combined(
    panel_data = panel_eu,
    targets = intersect(eu_plot_targets_use, names(panel_eu)),
    target_labels = eu_raw_plot_labels,
    tcodes = eu_tcodes,
  ...
  cat("\nSaved EU transformed series plot to:\n")
  cat(eu_transformed_plot_file, "\n")
}

cat("\nEU plot scope:\n")
cat(eu_plot_series_scope, "\n")

cat("\nNumber of series in combined plot:\n")
cat(length(eu_plot_targets_use), "\n")

# ============================================================
# STOP HERE IF ONLY PLOTS ARE REQUESTED
# ============================================================

if (isTRUE(eu_plots_only)) {
  eu_summary_file <- file.path(eu_dataset_dir, "eu_plots_only_summary.txt")
  
  cat(
    "EU plots-only export completed\n",
    "==============================\n\n",
    "Excel file:\n",
    eu_excel_path, "\n\n",
    "Dataset label:\n",
    ...
    eu_target_plot_dir, "\n\n",
    "Plot scope:\n",
    eu_plot_series_scope, "\n\n",
    "Targets:\n",
    paste(eu_target_order, collapse = ", "), "\n\n",
    "Combined plot variables:\n",
    paste(eu_plot_targets_use, collapse = ", "), "\n\n",
    file = eu_summary_file
  )
  
  cat("\nSaved EU plots-only summary file:\n")
  cat(eu_summary_file, "\n")
  
  cat("\nEU plots-only run finished.\n")
  ...
  cat(
    ...
    " - individual target plots in target_plots/\n",
    file = eu_summary_file
  )
